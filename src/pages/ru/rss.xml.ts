...
export async function GET(context: any) {
  ...
  const allPosts = await getCollection("blog", (post: CollectionEntry<"blog">) => {
    ...
    return !post.data.draft && lang === 'ru';
  });
  ...
  if (sorted.length === 0) {
    throw new Error("No valid blog posts for Russian RSS");
  }
    ...
    if (post.data.category) {
      try {
        ...
        if (categoryEntry?.data?.name) {
          ...
          categoryName = typeof name === 'string' ? name : (name as any)?.ru || (name as any)?.id || "Uncategorized";
        }
      } catch {}
    }
  ...
  const rssResponse = await rss({
    ...
    customData: `
      ...
      <atom:link href="${new URL('ru/rss.xml', site).toString()}" rel="self" type="application/rss+xml" />
    `,
    xmlns: {
