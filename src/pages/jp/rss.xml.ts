...
export async function GET(context: any) {
    ...
    if (post.data.category) {
      try {
        ...
        if (categoryEntry?.data?.name) {
          ...
          categoryName = typeof name === 'string' ? name : (name as any)?.jp || (name as any)?.id || "Uncategorized";
        }
      } catch {}
    }
  ...
  const rssResponse = await rss({
    ...
    description: siteData.defaultSeo?.jp?.description ?? '',
    site,
    items,
    customData: `
      ...
      <atom:link href="${new URL('jp/rss.xml', site).toString()}" rel="self" type="application/rss+xml" />
    `,
    xmlns: {
